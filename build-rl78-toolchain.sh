...
JEV_BINUTILS=binutils-2.35
JEV_GDB=gdb-9.2
JEV_ISL=isl-0.22.1

JEV_XTOOL_PREFIX=/opt/rl78-smoll
...
../${JEV_BINUTILS}/configure --prefix=${JEV_XTOOL_PREFIX} --enable-languages=c --target=rl78-elf
make -j8 all
make -j8 install
popd
...
mkdir -p build-gcc
pushd build-gcc
../${JEV_GCC}/configure --prefix=${JEV_XTOOL_PREFIX} --enable-languages=c --target=rl78-elf --without-headers --with-newlib --with-gnu-as --with-gnu-ld
make -j8 all-gcc
make install-gcc
popd
...
pushd build-newlib
../${JEV_NEWLIB}/configure --target=rl78-elf --prefix=${JEV_XTOOL_PREFIX}
make -j8 all
make install
popd

pushd build-gcc
../${JEV_GCC}/configure --prefix=${JEV_XTOOL_PREFIX} --enable-languages=c --target=rl78-elf --with-newlib --with-gnu-as --with-gnu-ld
make -j8 all
make install
popd
...
mkdir -p build-gdb
pushd build-gdb
../${JEV_GDB}/configure --prefix=${JEV_XTOOL_PREFIX} --enable-languages=c --target=rl78-elf
make -j8 all
make -j8 install
popd
